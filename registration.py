...
async def send_reply(req: Scroll, config: ConfigT, kinsfolk: KinsfolkT):
...
async def one_time_request(ctx: ContextT, client: ScrollT, server: ScrollT):
    ...
    if not (srv_endpoint := server.endpoint):
        raise ValueError(f"no registration endpoint found for {srv_name}")
    ...
    while True:
        ...
        # send the registration request
        if log_it := now > last_log + DEFAULT_RGSTR_LOG_INTERVAL:
            logger.info("... sending registration request to %s", server)
